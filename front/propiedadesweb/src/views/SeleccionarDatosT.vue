  ...
  <div class="seleccionar-terreno">
    ...
    <div class="entrada-datos-terreno">
      ...
      <div class="patio-option">
        ...
        <div class="botones">
          <button @click="establecerServicios(true)"
            :class="{ active: datosTerreno.tieneServicios === true }">Sí</button>
          <button @click="establecerServicios(false)"
            :class="{ active: datosTerreno.tieneServicios === false }">No</button>
        </div>
      </div>
    </div>
  ...
  setup() {
    ...
    const verificarDireccion = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8080/inmuebles/verificar-direccion?direccion=${datosTerreno.value.linea1}`);
